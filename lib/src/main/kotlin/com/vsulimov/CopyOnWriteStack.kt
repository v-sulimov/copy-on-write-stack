...
class CopyOnWriteStack<T>(private val elements: List<T> = emptyList()) {
     ...
     */
    fun push(element: T): CopyOnWriteStack<T> {
        return CopyOnWriteStack(elements + element)
    }
     ...
     */
    fun pop(): Pair<T?, CopyOnWriteStack<T>> {
     ...
     */
    fun mapInPlace(transform: (T) -> T): CopyOnWriteStack<T> {
        return CopyOnWriteStack(elements.map(transform))
    }
}
